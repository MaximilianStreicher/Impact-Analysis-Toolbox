...
%
% Script: Filtering of pressure sensor signals
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%
%
...
dir = '.\Input\Pressure\';                                                  % input directory
file = '#bi_01_5_DR00x15-1kHz.asc';                                         % input .txt file with pressure sensor signals (nr. of colums equals the number of pressure sensors)
...
lpf = 60;                                                                   % low pass filter [Hz]. Set to 0 if no low pass filter is used
lbpf = [49];                                                                % lower boundaries of bandpass filters [Hz]. Set to 0 if no bandwidth filter is used
ubpf = [51];                                                                % upper boundaries of bandpass filters [Hz]. Set to 0 if no bandwidth filter is used
noise = [-0.0035 -0.0035 0 0 0.005 0.0005 0.0015 0.0005...                  % upper noise boundary for each pressure sensor signal. All values below the upper noise boundary will be used to calculate the polynomial fit through the data in order to do the zero correction of the signal
         0.0005 0.0005 0.0005 0.001 0.0008 0.0005 0.0005];

     
%% LOAD DATA

in = dlmread([dir file],del,skiprows,skipcol);                              % read in pressure data from file
time = (0:1/fs:(length(in)-1)/fs)';                                         % generate time vector

%% FILTERING

for i = 1:size(in,2)                                                        % go over each pressure sensor individually
...
[p,s,mu] = polyfit(time(idx),detrY(idx),22);                                % fit polynomial best-fit line through selected data 
...
title('Raw data: Pressure signal');
legend('Time series measured pressure','FontSize',24);
hold off;
...
hold on; grid minor; 
plot(time,in(:,i),'-','Color',[0 0 0]);                                     % original signal                    
...
plot(time,f_y,'-','Color',[0 0.5 0]);                                       % best-fit polynom
...
fileID = fopen(['.\Output\Pressure\Sensor_' num2str(i)...
    '_filtered.txt'],'w');                                                  % save filtered time series
fprintf(fileID,'%s\t%s\n','Time [s]',['Sensor' num2str(i) ' [bar]']);
...
savefig(f1,['.\Output\Pressure\Sensor_' num2str(i) '_measured.fig']);       % save figures
savefig(f2,['.\Output\Pressure\Sensor_' num2str(i) '_spectrum.fig']);               
savefig(f3,['.\Output\Pressure\Sensor_' num2str(i)...
